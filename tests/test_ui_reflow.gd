...
func test_aspect_ratio_breakpoints() -> void:
	...
	assert_that(_SafeZoneManager.BREAKPOINT_SHRINK).is_equal(1.6)
	assert_that(_SafeZoneManager.BREAKPOINT_EXPAND).is_equal(1.9)


func test_safe_margins() -> void:
	var szm := AutoloadHelper.safe_zone_manager()
	if szm:
		var margins: Dictionary = szm.get_safe_margins()
...
func test_notch_offset() -> void:
	var szm := AutoloadHelper.safe_zone_manager()
	if szm:
		var offset: Vector2 = szm.get_notch_offset()
