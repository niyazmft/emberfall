...
func test_stats_update() -> void:
	var hud: Control = auto_free(load(COMBAT_HUD_SCENE).instantiate() as Control)
	add_child(hud)

	var entity: Entity = auto_free(Entity.new("TestPlayer", 0, 0, 100, 10, 5))
...
func test_turn_indicator() -> void:
	...
	var turn_manager: TurnManager = auto_free(TurnManager.new())
