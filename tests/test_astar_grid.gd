...
func test_wall_detour() -> void:
	...
	Engine.get_main_loop().root.get_node("GridSystem").load_room({"id": "wall", "tiles": wall_tiles})
	var astar: AStarGrid = AStarGrid.new()
	var path: PackedVector2Array = astar.find_path(Vector2i(0, 0), Vector2i(11, 11))

	assert_that(path.is_empty()).is_false()
	for i: int in range(path.size()):
		var tile: TacTileData = Engine.get_main_loop().root.get_node("GridSystem").get_tile(int(path[i].x), int(path[i].y))
		if tile != null:
...
func test_elevation_blocking() -> void:
	...
	Engine.get_main_loop().root.get_node("GridSystem").load_room({"id": "elev", "tiles": elev_tiles})
	var astar: AStarGrid = AStarGrid.new()
	...
	if not path.is_empty():
		for i: int in range(path.size() - 1):
			...
			var ta: TacTileData = Engine.get_main_loop().root.get_node("GridSystem").get_tile(int(a.x), int(a.y))
			var tb: TacTileData = Engine.get_main_loop().root.get_node("GridSystem").get_tile(int(b.x), int(b.y))
...
func test_corner_cutting_prevention() -> void:
	...
	Engine.get_main_loop().root.get_node("GridSystem").load_room({"id": "corner", "tiles": corner_tiles})
	var astar: AStarGrid = AStarGrid.new()
