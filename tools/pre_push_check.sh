...
for log in tools/godot_lint.log tools/test_suite.log; do
    if [ -f "$log" ]; then
        ...
        if grep "ERROR:" "$log" 2>/dev/null | grep -ivE "resources still in use|objectdb instances leaked|caller thread can't call this function|statemachine: attempted to change to unregistered state|Formula failed safety check|Invalid equipment slot"; then
            echo ""
