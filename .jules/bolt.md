...
## 2026-06-22 - Lambda capture of typed String locals fails silently in Godot 4.6.3

**Learning:** A lambda assigned to a `Callable` variable and connected to a signal silently fails to capture a typed `String` local variable by reference. The lambda body executes (`print` fires) but the assignment to the outer `String` variable is lost. Using an `Array` (a reference type) as the capture container works around the issue. This only affects typed locals; untyped or reference-type capture appears unaffected.

**Action:** When capturing signal results in test lambdas, use an `Array` or `Dictionary` container instead of a plain typed local variable. Prefer `received.append(value)` over `text_received = value` in signal callbacks.

## 2026-06-23 - Godot 4.6.3 shutdown crash is an engine bug, not test failures

**Learning:** When running the full GdUnit4 test suite (289 tests), Godot 4.6.3 crashes during `Main::cleanup()` with `EXC_BAD_ACCESS` in `GDScriptInstance::~GDScriptInstance()`. The crash stack trace shows: `Main::cleanup() → unregister_core_types() → Object::~Object() → GDScriptInstance::~GDScriptInstance() → recursive_mutex::lock()`. This happens AFTER all tests pass with 0 failures. The crash is caused by orphan nodes (48–67 counted by GdUnit4) that outlive the script language system teardown. These orphans are primarily from autoload re-initialization during tests (e.g., `BurdenManager._ready()` creating new `_MWTCaptionMatrix` instances that overwrite prior ones) and scene-instantiation tests that leave nodes behind. The crash is intermittent and is a known Godot 4.x shutdown-order fragility between `ObjectDB::cleanup()` and `GDScriptLanguage::finish()`.

**Action:** Do NOT chase every orphan as a bug. The test suite passes; the crash is an engine limitation. If CI needs a clean exit code, run tests individually per file instead of `gdUnit4 -a tests/`. To reduce crash probability, avoid calling autoload `._ready()` repeatedly in tests — use `before_test()`/`after_test()` to reset state instead of re-initializing singletons. Upgrade to Godot 4.7+ when available, which may fix the shutdown order.
