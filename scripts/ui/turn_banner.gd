...
func display_message(text: String, color: Color = Color.WHITE) -> void:
	...
	# Slide in from left
	position.x = -size.x
	tween.tween_property(self, "position:x", 0.0, _slide_duration).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	...
	fade_out.tween_property(self, "position:x", size.x, _slide_duration).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_IN)
...
func _on_combat_started(_room_index: int, _room_data: Dictionary) -> void:
	display_message(tr("HUD_COMBAT_START") if TranslationServer.has_message("HUD_COMBAT_START") else "COMBAT START", Color.ORANGE)


func _on_turn_started(entity: Entity, is_player: bool) -> void:
	if is_player:
		display_message(tr("HUD_PLAYER_TURN_BANNER") if TranslationServer.has_message("HUD_PLAYER_TURN_BANNER") else "PLAYER TURN", Color.GREEN)
	else:
		var enemy_name: String = entity.entity_name if entity else "ENEMY"
		var msg: String = tr("HUD_ENEMY_TURN_BANNER") % enemy_name if TranslationServer.has_message("HUD_ENEMY_TURN_BANNER") else enemy_name + " TURN"
