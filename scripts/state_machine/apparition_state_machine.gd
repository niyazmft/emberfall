...
func _register_states() -> void:
	register_state(ApparitionState.INACTIVE, &"INACTIVE",
		Callable(self, "_enter_inactive"), Callable(), Callable())
	register_state(ApparitionState.MANIFEST, &"MANIFEST",
		Callable(self, "_enter_manifest"), Callable(), Callable(self, "_update_manifest"))
	register_state(ApparitionState.IDLE, &"IDLE",
		Callable(self, "_enter_idle"), Callable(), Callable(self, "_update_idle"))
	register_state(ApparitionState.RECOIL, &"RECOIL",
		Callable(self, "_enter_recoil"), Callable(self, "_exit_recoil"), Callable(self, "_update_recoil"))
	register_state(ApparitionState.ABSORB, &"ABSORB",
		Callable(self, "_enter_absorb"), Callable(), Callable(self, "_update_absorb"))
	register_state(ApparitionState.ERROR, &"ERROR",
...
func _register_transitions() -> void:
	...
	register_transition(ApparitionState.MANIFEST, ApparitionState.IDLE,
		Callable(self, "_guard_manifest_done"), Callable())

	# IDLE → RECOIL (guard: recoil command received)
	register_transition(ApparitionState.IDLE, ApparitionState.RECOIL,
		Callable(self, "_guard_recoil_triggered"), Callable())

	# RECOIL → IDLE (guard: recoil timer elapsed)
	register_transition(ApparitionState.RECOIL, ApparitionState.IDLE,
		Callable(self, "_guard_recoil_done"), Callable())

	# IDLE → ABSORB (guard: absolve / burden deactivated)
	register_transition(ApparitionState.IDLE, ApparitionState.ABSORB,
		Callable(self, "_guard_absolve_triggered"), Callable())

	# ABSORB → INACTIVE (guard: absorb timer elapsed)
	register_transition(ApparitionState.ABSORB, ApparitionState.INACTIVE,
		Callable(self, "_guard_absorb_done"), Callable())

	# ERROR → INACTIVE (recovery)
	register_transition(ApparitionState.ERROR, ApparitionState.INACTIVE,
		Callable(), Callable())
