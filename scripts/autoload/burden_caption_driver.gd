...
func _connect_middleware() -> void:
	...
	if am and am.has_signal("stem_event_detected"):
...
func _map_and_trigger_caption(stem_id: String, event_type: String, intensity: float) -> void:
	...
	if not caption_text.is_empty():
		var cm := get_node_or_null("/root/CaptionManager")
		if cm and cm.has_method("schedule"):
			# Channel.BURDEN = 1
		...
		if cm and cm.has_method("report_stem_transient"):
