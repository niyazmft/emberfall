...
## ── Dependencies (use preload for robustness in Autoloads) ──────
const DeterministicMath = preload("res://scripts/core/deterministic_math.gd")
...
## Internal Classes
var _event_engine: BurdenEventEngine = null
var _caption_bridge: BurdenCaptionBridge = null

## MWT Matrix (DON-222)
const MWTCaptionMatrix = preload("res://ui/framework/mwt_caption_matrix.gd")
var _mwt_matrix: MWTCaptionMatrix = null
...
func _ready() -> void:
	_mwt_matrix = MWTCaptionMatrix.new()
	_load_burden_config()
...
func update_moral_weight(moral_flag: int) -> void:
	...
	var new_level: int = clampi(floori(float(moral_flag) / float(threshold) * 3.0), 0, 3)
